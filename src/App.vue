...
export default {
    ...
    async mounted() {
        ...
        Promise.all(instruments.map((i) => i.load())).then(() => {
            if (!getQueryVariable("fromURL")) {
                ...
                fetch(getQueryVariable("fromURL") as string)
    ...
    computed: {
        ...
        tabTitle() {
            ...
            const base = this.state.song.name ? `${this.state.song.name} - WebNBS` : "WebNBS";
    ...
    methods: {
         ...
         */
        advanceSong(time: number, timePassed: number) {
            ...
            for (const layer of song.layers) {
                ...
                if (note && !layer.locked) {
                    this.state.playNote(note, layer);
        ...
        tick() {
            ...
            if (this.$refs.canvas) {
                (this.$refs.canvas as any).update(time);
            }
        },
    },
};
