...
export default {
    ...
    computed: {
        currentTime() {
            return this.formatTime(this.song?.currentTime || 0);
        },
        endTime() {
            return this.formatTime(this.song?.endTime || 0);
        },
        isVanillaFriendlyTempo() {
            return VANILLA_FRIENDLY_TEMPOS.includes(this.song?.tempo || 0);
        },
    },
    ...
    methods: {
        ...
        focusTempo() {
            (this.$refs.tempo as HTMLInputElement).focus();
        },
    },
};
</script>
