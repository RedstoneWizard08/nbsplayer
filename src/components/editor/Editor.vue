...
import * as NBS from "@/lib/nbs";
import { SongEditor } from "./editor.js";
import { NOTE_SIZE } from "./config.js";
import * as Objects from "./objects";
import { useAppState } from "@/stores/app";
import { useOptions } from "@/stores/options";
import noteImage from "@/assets/instruments/textures/note.png";

const NOTE_IMG = new Image();

NOTE_IMG.src = noteImage;
...
export default {
    ...
    data() {
        return {
             ...
             */
            textureCache: {} as Record<string, HTMLCanvasElement>,
             ...
             */
            boundingRects: null as DOMRect | null,

            /**
             * Objets within this editor.
             */
            objects: [] as Objects.EditorObject[],

            /**
             * The object currently being interacted with, if any.
             */
            interaction: null as Objects.EditorObject | null,
    ...
    methods: {
        ...
        handleMouse(e: MouseEvent) {
            if (e.type === "mouseup" || e.type === "mousedown") {
                ...
                if (this.interaction) {
                    ...
                    this.interaction.dragged(this as Objects.VM, dx, dy);
                }
            }
        },
        ...
        endInteraction() {
            ...
            this.interaction.interactEnd(this as Objects.VM);
        ...
        findInteraction(button: number) {
            ...
            while (i--) {
                ...
                if (object.intersectsPoint(this.mouse)) {
                    const interaction = object.interact(this as Objects.VM, button);
        ...
        updateObjects() {
            for (const object of this.objects) {
                object.update(this as Objects.VM);
            }
        },
         ...
         */
        drawNotes(time: number) {
            /**
             * Creates a texture for a noteblock with a given instrument and key.
             */
            const createNoteTexture = (instrument: NBS.Instrument, key: number) => {
                ...
                ctx.drawImage(this.options.coloredBlock ? instrument.baseTexture! : NOTE_IMG, 0, 0);
            ...
            const _visibleTicks = this.editor?.viewport.width;
            const start = this.editor?.viewport.firstTick || 0;
            const end = this.editor?.viewport.lastTick || 0;
